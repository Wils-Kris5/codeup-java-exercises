...
//```
//
//    We could now add a main method to actually run the code. When we run the code, we have to actually create a Fighter. This is not an actual fighter object, it's just a blueprint for how to create a fighter object. To make an actual fighter that instantiates the object, we could make a main method like this:
//
//            ```
...
//
//    We have seen this before, specifically in the Math class. All the methods in the Math class are static and we can call them directly from the class itself without instantiating it. [The Math Class](https://docs.oracle.com/en/java/javase/11/docs/api/java.base/java/lang/Math.html). Notice all the methods in the Math class are static.
//
//            ```
...
//```
//
//    You call a static method using a class name. You call a non-static method using a reference variable name.
//
...
//
//    We can also have static variables. Now you may have noticed something about your main method, which is of course a static method. The main method can never use an instance variable.
//
//    Does not compile:
//            ```
//    public class MathTest {
...
//```
//    The reason for this is that an instance variable by definition is meant to be instantiated, to belong to a specific object and to receive a specific value. However, a static method is meant to run off a class, not an instance of that class. So, static methods cannot use instance variables.
//
//    On the other hand, there is such a thing as a static variable. The value of a static variable is the same for all instances of the class. All instances of the class share a single copy of a static variable. And when all instances of the class share a single copy of the static variable, that means that we can call it from the class rather than only from instances.
//
//    Club.java:
//            ```
//    public class Club {
...
//```
//
//    So this is how we can have access to variables from within the main method, and access to some variables throughout a Java program.
//
...
//```
//
//    Notice what it is missing; no return type. That's how you know it's a constructor. Aside from that, its name is the name of a class - that's mandatory.
//
...
//            ## Visibility
//
//    It is at this stage that we get into one of the most important aspects of Java programming, access levels and access modifiers. This is also called visibility, because it affects who sees what.
//
//    So far, we've been either just declaring an instance variable ```int myInt = 42``` or declaring it public ```public int myInt = 42```. What does this ```public``` keyword mean anyway? Or the complete absence of a keyword at all?
//
//    Public means any code anywhere can access it. If you are writing a program with a bunch of classes, you can access this. If you are importing packages, you can access public properties inside the packages, and they can access yours.
//
//    Default -- which is what happens when you don't declare something -- means only code within the same package can access a property / method / variable. usually, a package is designed as a coherent group of classes that work together as a set, so it does make sense to have classes in the same package access one another's code, while only a few of the classes are exposed to code outside the package.
//
//    On the other hand, there is the ```private``` level of access, this level means only code within the same class can access it. It is private to the class.
//
//    In practice, we use this a lot for getters and setters and encapsulation. So, right now we could alter Arata's code in the FighterTest class.
//
//    FighterTest.java:
//            ```
//    Fighter arata = new Fighter("Arata", 17, 14);
...
//```
//    What we actually want to do is to route any changes to instance variables through a method so that we can include validation code and make sure this is something we really want to happen. We say we encapsulate the data. Let's make that happen.
//
//    Fighter.java
//```
//    private String name;
...
//```
//
//    Notice I don't need to modify the constructor to use the setter, because the constructor and the private variable are in the same class. But now if I try to do ```arata.hitPoints = 0;``` in FighterTest, I get ```java: hitPoints has private access in Fighter```. I have to do
//            ```
//    Fighter arata = new Fighter("Arata", 17, 14);
...
//```
//
//        These methods are called getters and setters, or more technically mutators, but almost everyone calls them getters and setters. Proper Java coding technique calls for class instance variables to be private, and accessible only through getters and setters.
//
//### mini-exercise
//        Encapsulate your animal class. Use your test class to verify that your instance variables cannot be directly accessed and can only be accessed through your getters and setters.
//
  // a static variable is more associated with the class
    // void is a method to mutate data
    // setters what you intentionally set something
    //gettters are for specifically getting something
    // constructor is used as blue print that uniquly contaains this values unique to its own instaciation
